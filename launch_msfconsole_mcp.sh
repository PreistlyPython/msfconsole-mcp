...
# Enhanced Launch script for MSFconsole MCP
...
echo -e "${BOLD}${BLUE} MSFConsole MCP Launcher${RESET}"
...
if command -v msfconsole &> /dev/null; then
    ...
    # Don't run version check here as it might hang
else
    warn "msfconsole not found in PATH. Some functionality will be limited."
fi

# Run the MCP server with proper error handling
echo -e "${BOLD}${BLUE}==========================================${RESET}"
echo -e "${BOLD}${BLUE} Starting MSFConsole MCP server...${RESET}"
echo -e "${BOLD}${BLUE}==========================================${RESET}"

# Run with capturing output but allow Ctrl+C to work properly
if ! python -u msfconsole_mcp.py; then
    error "MSFConsole MCP failed to start. Check the logs for details."
    exit 1
fi
